...
def get_data_from_dir(data_dir, start_date, end_date):
    file_list = [i for i in os.listdir(data_dir) if os.path.isfile(os.sep.join([data_dir, i]))]
    s = pd.DataFrame()

    for i in file_list:
        idxfile = os.sep.join([data_dir, i])
        # print idxfile
        if s.empty:
            s = pd.read_csv(idxfile)
        else:
            s = s.append(pd.read_csv(idxfile))
    return s
